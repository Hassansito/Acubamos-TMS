<template>
    <div class="card">
        <div class="card-header">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" v-model="search" @input="searchItems()"
                    class="form-control form-control-solid w-250px ps-15" placeholder="Buscar Contrato" />
            </div>
            <div class="card-toolbar">
                ...
                <div v-else class="d-flex justify-content-end align-items-center">
                    ...
                    <button type="button" class="btn btn-danger" @click="deleteFewcontratos()">
                        Delete Selected
                    </button>
                </div>
            </div>
        ...
        <div class="card-body">
            <KTDatatable :header="tableHeader" :data="contratos" @on-sort="sort" :checkbox-enabled="true"
                @on-items-select="onItemSelect">
                <template v-slot:id="{ row: data }">
                    {{ data.id }}
                </template>
                <template v-slot:cliente="{ row: data }">
                    {{ data.cliente }}
                </template>
                ...
                <template v-slot:contrato="{ row: data }">
                    <a v-if="data.contrato" :href="data.contrato" target="_blank"
                ...
                <template v-slot:actions="{ row: data }">
                    <div>
                        <a @click="deleteContrato(data.id)" class="btn btn-sm btn-light btn-active-light-danger">Delete
                        </a>
                    </div>
                </template>
            </KTDatatable>
...
export default defineComponent({
    ...
    setup() {
        ...
        const initCustomers = ref<Array<IIContratos>>([]);
        console.log("Contratos en el store:", contratos.value);
        onMounted(() => {
        ...
        const searchItems = () => {
            ...
            if (search.value !== "") {
                let results: Array<IContratociones> = [];
